...
class LogoutPageViewTestCase(TestCase):
    ...
    def test_checkout_logout_message(self):
        self.assertContains(self.response, 'Thanks for spending some quality time with us today.')
...
class RegisterPageViewTestCase(TestCase):
    ...
    def test_register_email_content_correct(self):
        _ = self.register()
        letter = mail.outbox[0]
        self.assertTrue("Click here to complete your Registration" in letter.message().get_payload())
    ...
    def test_register_email_link_works(self):
        self.register()
        letter = mail.outbox[0]
        email_link = '/' + letter.message().get_payload().split('"')[1].replace('testserver/', '')
...
class TemplateDefaultViewTestCase(TestCase):
    ...
    def test_index_contains_proper_static_links(self):
        self.assertContains(self.response, '/static/images_api/assets/css/main.css')

    def test_index_handles_lt_ie8(self):
        self.assertContains(self.response, '/static/images_api/assets/js/ie/html5shiv.js')
        self.assertContains(self.response, '/static/images_api/assets/js/ie/respond.min.js')
        self.assertContains(self.response, '/static/images_api/assets/css/ie8.css')
        self.assertContains(self.response, '/static/images_api/assets/css/ie9.css')

    def test_javascript_libraries_load(self):
        self.assertContains(self.response, '/static/images_api/assets/js/jquery.min.js')
        self.assertContains(self.response, '/static/images_api/assets/js/skel.min.js')
        self.assertContains(self.response, '/static/images_api/assets/js/main.js')
    ...
    def test_login_button_appears(self):
        self.assertContains(self.response, 'Login</a>')

    def test_register_button_appears(self):
        self.assertContains(self.response, 'Register</a>')
...
class OtherProfileViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.response = self.client.get(resolve_url('images:self_view'))
        self.response2 = self.client.get(resolve_url('images:profile_view', profile_id=self.user2.id))
    ...
    def test_profile_url_is_correct(self):
        self.assertContains(self.response, "Images</a></li>")

    def test_profile_name_appears(self):
        self.assertContains(self.response, 'kent')

    def test_other_profile_appears(self):
        self.assertContains(self.response2, 'norton')
